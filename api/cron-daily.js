// /api/cron-daily.js
// 每天 PST 7am 自动跑：RSS → Claude 选稿 → 生成 Founder Notes → Resend 邮件
// 也可手动 POST 触发
//
// 需要的 env vars:
...
function parseResponse(xml) {
  ...
  const titleParts = [1, 2, 3, 4].map(i => ext(content, `title_line_${i}`)).filter(Boolean);
  const title = titleParts.join('').replace(/\*\*/g, '');

  const hook = ext(content, 'hook');
  const socialTitle = ext(content, 'social_title');
  const socialBody = ext(content, 'social_body');
  const socialTags = ext(content, 'social_tags').split('\n').map(t => t.trim().replace(/^#+/, '')).filter(Boolean);
  ...
  return { title, hook, socialTitle, socialBody, socialTags, chapters };
}
...
async function sendEmail({ to, parsed, seed }) {
  ...
  const subject = (parsed.socialTitle || parsed.title || '今日 Founder Notes').replace(/\n/g, ' ');
  const tagLine = parsed.socialTags.map(t => '#' + t).join(' ');
  ...
  const html = `<div style="font-family: -apple-system, 'PingFang SC', sans-serif; line-height: 1.7; color: #1a1a1a; max-width: 640px;">
    ...
    <pre style="white-space: pre-wrap; font-family: inherit; font-size: 15px; margin: 0 0 18px 0;">${escapeHtml(parsed.socialBody || parsed.hook || '')}</pre>
...
export default async function handler(req, res) {
  ...
  try {
    ...
    await sendEmail({ to, parsed, seed });
